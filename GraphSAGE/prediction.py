...
from GraphSAGE.graphsage.inits import zeros
from GraphSAGE.graphsage.layers import Layer
...
class BipartiteEdgePredLayer(Layer):
    ...
    def _xent_loss(self, inputs1, inputs2, neg_samples, hard_neg_samples=None): # pointwise loss
        print('point_loss')
    ...
    def _hinge_loss(self, inputs1, inputs2, neg_samples, hard_neg_samples=None): # pairwise loss
        print('pairwise_loss')
        aff = self.affinity(inputs1, inputs2)
        neg_aff = self.neg_cost(inputs1, neg_samples, hard_neg_samples)
