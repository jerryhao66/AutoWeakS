...
import GraphSAGE.graphsage.layers as layers
import GraphSAGE.graphsage.metrics as metrics

from GraphSAGE.graphsage.prediction import BipartiteEdgePredLayer
from GraphSAGE.graphsage.aggregators import MeanAggregator, MaxPoolingAggregator, MeanPoolingAggregator, SeqAggregator, GCNAggregator
from GraphSAGE.graphsage.setting import setting
...
class SampleAndAggregate(GeneralizedModel):
    ...
    def _build(self):
        ...
        self.neg_samples, _, _ = (tf.nn.fixed_unigram_candidate_sampler(
            true_classes=labels,
            num_true=1,
            num_sampled=self.FLAGS.neg_sample_size,
            unique=False,
        ...
        neg_samples, neg_support_sizes = self.sample(self.neg_samples, self.layer_infos,
            self.FLAGS.neg_sample_size)
        self.neg_outputs, _ = self.aggregate(neg_samples, [self.features], self.dims, num_samples,
                neg_support_sizes, batch_size=self.FLAGS.neg_sample_size, aggregators=self.aggregators,
                concat=self.concat, model_size=self.model_size)

        dim_mult = 2 if self.concat else 1
        self.link_pred_layer = BipartiteEdgePredLayer(dim_mult*self.dims[-1],
                dim_mult*self.dims[-1], self.placeholders, act=tf.nn.sigmoid, loss_fn=self.FLAGS.loss_function,
    ...
    def _accuracy(self):
        ...
        self.neg_aff = tf.reshape(self.neg_aff, [self.batch_size, self.FLAGS.neg_sample_size])
...
class Node2VecModel(GeneralizedModel):
    ...
    def _build(self):
        ...
        self.link_pred_layer = BipartiteEdgePredLayer(self.hidden_dim, self.hidden_dim,
                self.placeholders, loss_fun=self.FLAGS.loss_function, bilinear_weights=False)
        print('excute hinge loss function...')
    ...
    def _loss(self):
        if FLAGS.loss_function =='xent': #pointwise
