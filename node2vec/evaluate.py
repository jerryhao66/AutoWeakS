from node2vecalgo.setting import setting
import numpy as np
import math
import heapq
...
def evaluate(vectors, args):
    ...
    for i in range(len(job_list)):
        ...
        job_ebd = vectors[job_list[i]]
        for item in candidate_courses_list:
            course_ebd = vectors[item]
    ...
    final_hr20, final_ndcg20, final_hr10, final_ndcg10, final_hr5, final_ndcg5, final_map, final_mrr = np.array(hits20).mean(), np.array(
        ndcgs20).mean(), np.array(hits10).mean(), np.array(ndcgs10).mean(), np.array(hits5).mean(), np.array(ndcgs5).mean(), np.array(maps).mean(), np.array(mrrs).mean()
...
if __name__ == '__main__':
    args = setting()
    hr20, ndcg20, hr10, ndcg10, map_, mrr = evaluate(vectors, args)